...
/**
 * @fn errorLevel CreateRestriction(relationship proximity, double min_rating,
 * double max_rating, userRestriction *restriction)
 * @brief Função que cria uma restrição de usuário.
 * @param proximity Proximidade com o usuário desejada.
 * @param min_rating Avaliação mínima do usuário desejado.
 * @param restriction Endereço da estrutura de dados onde a restrição será
 * criada. Argumento passado por referência.
 * @return A função retorna uma instância do tipo errorLevel: Success caso a
 * restrição seja criada com sucesso; Illegal_argument caso a função receba
 * um ponteiro nulo como argumento.
 *
 * Copia os argumentos fornecidos à função para o endereço da estrutura de
 * dados do tipo userRestriction fornecida como argumento.
 *
 * Assertivas de entrada:
 *  -O parâmetro restriction deve ser diferente de NULL.
 *  -O parâmetro restriction deve apontar para uma estrutura de dados do
 * tipo userRestriction.
 *
 * Assertivas de saída:
 *  -Os membros da restrição no endereço restriction serão aqueles passados como
 ...
 *
 * Assertivas de contrato:
 *  -As informações contidas no endereço restriction serão sobrescritas
 ...
 *
 * Hipóteses:
 *  -O banco de dados de usuários está configurado corretamente.
 *
 */
