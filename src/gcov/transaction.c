 ...
 * passado para a função seja uma transação que não p um ponteiro NULL.
 *
 ...
 * dados do tipo transactionStatu. Os dados em new_transaction são sobrescritos
...
errorLevel CancelTransaction(transaction *target_transaction) {
  ...
  /*
    Uma transação só pode ser cancelada se o seu status é Open ou InProgress,
    pois não faz sentido cancelar transações já finalizadas (status = Closed),
    transações já canceladas (status = Canceled) ou transações que geraram erros
    (status = Error).
   */
...
errorLevel UpdateTransaction(unsigned int user_id,
  ...
  /*
    Só se pode atualizar transações de status "Open" e nunca pode-se fazê-lo com
    o mesmo usuário que já estava na transação ou com o id 0, por ser um id de
    usuário inválido nesse contexto.
   */
